...
class ResultController extends AppBaseController
{
    ...
    public function show($id)
    {
        ...
        $debug          = array('quadran individual D' => $disc_pattern_D,
    ...
    public function exportExcel() {
        $competency         = CategoryCompetency::All();
        $user_assessment    = Result::userAssessmentResult();
        $data               = array();   
        ...
        echo '<table border="1">';
        echo '<tr><th>NIK</th><th>NAMA</th><th>JABATAN</th>';

        foreach ($competency as $compt) {
            echo '<th>'.$compt->alias.'</th>';
        }

        echo '<th>RESULT</th>';
        echo '</tr>';
        foreach ($user_assessment as $user) {

            echo '<tr>';
            echo '<td>'.$user->nik.'</td>';
            echo '<td>'.$user->nama.'</td>';
            echo '<td>'.$user->position_name.'</td>';
            $result     = 0;
            $compt_tot  = 0;
            foreach ($competency as $compt) {
                $det_user_assessment = Result::detailUserAssessmentResult($user->nik, $compt->id, $user->last_test);
                if(empty($det_user_assessment)){
                    echo '<td></td>';
                }else{
                    echo '<td>'.$det_user_assessment->value.'%</td>';
                    $result += $det_user_assessment->value;
                    $compt_tot++;
                }

            }
            echo '<td>'.round($result/$compt_tot).'%</td>';
            echo '</tr>';
        }
        echo '</table>';
