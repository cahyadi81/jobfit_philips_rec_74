...
class Result extends Model
{
    ...
    public function getTestDateAttribute(){
        return Carbon::parse($this->last_test)->format('d-F-y');
    }
    ...
    public static function getNikResult($nik, $last_test){
        return DB::table('assess_result as a')
                    ->select('a.nik','b.flag_agreement_score','b.nama', 'b.jabatan as position_id', 'c.position_name as jabatan', 'a.last_test', 'b.departement', 'b.divisi', 'b.area', 'b.result_disc')
                    ->groupBy('a.nik', 'a.last_test')
                    ->leftJoin('user_assesment as b', 'a.nik', '=', 'b.nik')
                    ->leftJoin('positions as c', 'b.jabatan', '=', 'c.id')
    ...
    public static function indvDiscPattern($value){
        return DB::table('quadran_individuals')
                    ...
                    ->whereRaw($value. '>= min AND '.$value.'<= max')
                    ->first();
    }
    ...
    public static function getFitScoreNew($rata){
        return DB::table('agreement_scores_2')
                    ->select('norma')
                    ->where('agreement_score', $rata)
                    ->first();
    }
    ...
    public static function jobfitBasic($position_id, $jobfit_id)
    {
        return DB::table('jobfit_basics')
                    ->distinct('pattern')
                    ->where(['position_id' => $position_id, 'jobfit_id' => $jobfit_id])
                    ->limit('1')
                    ->first();
    }

    public static function userAssessmentResult(){
        return DB::table('user_assesment_result as a')
                    ->select('a.nik','a.result', 'b.nama', 'a.last_test', 'c.position_name', 'b.departement', 'b.divisi', 'b.area', 'b.email', 'b.no_telp', 'b.result_disc', 'd.tipe_dev')
                    ->leftJoin('user_assesment as b', 'a.nik', '=', 'b.nik')
                    ->leftJoin('positions as c', 'b.jabatan', '=', 'c.id')
                    ->leftJoin('assess_report_header as d', 'b.result_disc', '=', 'd.kode_dev')
                    ->groupBy('a.nik', 'a.last_test')
                    ->get();
    }
}
